var questions = [
    {
        question: "What does the acronym CPU stand for in computer science?",
    ...
    },
    {
        ...
        answer: "Store data temporarily for quick access",
        options: [
    ...
    {
        question: "Which language is commonly used for developing web pages?",
        answer: "HTML",
        options: [
            "Python",
            "HTML",
  ...
  function loginButton() {
    ...
    if (username.value === "sam" && password.value === "123") {
    ...
    for (var j = 0; j < questions.length; j++) {
        ...
        for (var i = 0; i < numOfOptions; i++) {
            if (optionVals[i] === answerVal)
                ...
                divBody += `<li class='optionPanel'>${optionVals[i]}</li>`//options which don't match the answer
        }
  ...
  function addRec(id) {
    ...
    if (!(questionVal === '') && !(answerVal === '')) {
        ...
        if (enteredOptions !== 2) alert("Please enter at least two options")
        else {
            enteredOptions = 0
            for (var i = 0; i < optionVals.length; i++) {
                if (optionVals[i] === answerVal) {
            ...
            if (!enteredOptions) alert("Please enter one of the options in answer field")
            else {
                addingRec = false
                if (editingRec) {
                    editingRec = false
